...
describe("GET /teams/:id", () => {
    it("doit retourner une équipe existante", async () => {
      ...
      expect(Array.isArray(res.body)).toBe(true); 
      expect(res.body.length).toBeGreaterThan(0);
      // expect(res.body).toHaveProperty("id", 1); pourquoi ça ne fonctionne pas?
    });
...
describe("POST /teams", () => {
    ...
    beforeEach(async () => {
        await Team.destroy({ where: {}, truncate: true, cascade: true, restartIdentity: true });
    });
...
describe("DELETE /teams/:id", () => {
    ...
    beforeEach(async () => {
        ...
        pokemonForTeam = await Pokemon.create({ name: `PkmForTeam_${Date.now()}`, hp: 10, atk: 10, def: 10, atk_spe: 10, def_spe: 10, speed: 10});
    ...
    it("doit supprimer une équipe existante et retourner un statut 201", async () => {
        ...
        expect(res.statusCode).toBe(201);
        expect(res.body.id).toBe(teamToDelete.id); 
        ...
        // Vérifie que le Pokémon existe mais n'est plus associé à cette équipe 
        const pokemonStillExists = await Pokemon.findByPk(pokemonForTeam.id);
        expect(pokemonStillExists).not.toBeNull(); 
    });
...
describe("PUT /teams/:teamid/pokemons/:pokemonid (Add Pokemon to Team) - Concise Version", () => {
    ...
    let freshTeam, testPokemon, teamFull, pokemonNotInTeam;

    beforeAll(async () => {
        testPokemon = await Pokemon.create({ name: 'Concise_Pkm', hp: 10, atk: 10, def: 10, atk_spe: 10, def_spe: 10, speed: 10});

        pokemonNotInTeam = await Pokemon.create({ name: 'New_Pkm_For_Add', hp: 20, atk: 20, def: 20, atk_spe: 20, def_spe: 20, speed: 20 });

        teamFull = await Team.create({ name: 'Concise_Full_Team' });
        for (let i = 1; i <= 6; i++) {
            const pkm = await Pokemon.create({ name: `Full_Pkm_${i}`, hp: 1, atk: 1, def: 1, atk_spe: 1, def_spe: 1, speed: 1});
